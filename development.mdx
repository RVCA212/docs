...
Easily monitor your API usage (excerpt from `architect.py`):
```python
...
Run long-running tasks without blocking your application (see `headless_example copy.py`):
```python
...
sdk = SandboxSDK(
    model="gpt-4.1", api_key=os.getenv("CLOUD_CODE_API_KEY")
)
...
sdk.run_command("cd /home/user/project && python -m pytest")
```
