...
| **Multi-Step** | Related task sequence | Plan → Implement → Test |
| **Tool-Controlled** | Specific operations | Read-only analysis, then editing |
| **Task Planning** | Complex feature development | Plan → Implement → Test → Deploy |
| **Directory-Focused** | Large codebase work (separate calls) | Component migration per directory |

<Info>
**Note**: Directory-focused patterns require separate API calls due to the `workingDir` + `continueConversation` limitation.
</Info>
